...
const MobileNav = ({ closeNav, showNav }: Props) => {
  ...
  return (
    <div>
      ...
      {/* NAVLINKS */}
      <div
        className={`text-white ${navOpenStyles} fixed top-0 left-0 flex flex-col justify-center h-screen w-[80%] bg-[#b96974] space-y-5 z-[1050]`}
      >
        {NAVLINKS.map((link) => (
          <button
            key={link.id}
            ...
            className="ml-12 text-left text-[20px] sm:text-[28px] font-medium hover:text-gray-200 transition-all duration-300 cursor-pointer"
          >
            {link.label}
          </button>
        ))}
